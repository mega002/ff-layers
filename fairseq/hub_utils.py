...
class GeneratorHubInterface(nn.Module):
    ...
    def score(self, sentences: List[str], extract_mode: str, **kwargs):
        if isinstance(sentences, str):
            return self.score([sentences], **kwargs)[0]
        # NOTE: this doesn't support translation tasks currently
        tokenized_sentences = [self.encode(sentence) for sentence in sentences]
        return [hypos[0] for hypos in self.generate(tokenized_sentences, extract_mode, score_reference=True, **kwargs)]
    ...
    def generate(
        ...
        **kwargs
        ...
        gen_args.beam = beam
        gen_args.extract_mode = extract_mode
